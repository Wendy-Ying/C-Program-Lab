...
int convert(unsigned char *RawData, unsigned char *RData, unsigned char *GData, unsigned char *BData, unsigned int row, unsigned int col);

int main(int argc, char *argv[]) {
	unsigned char *Raw, *R, *G, *B;
	...
	scanf("%d", &Col);
	tmp = Row * Col;
	R = (unsigned char *)malloc(tmp * sizeof(unsigned char));	//scanf输入的必须是4字节的int，所以分配int类型对象，实际图像存储中，是以unsigned byte保存的 
	G = (unsigned char *)malloc(tmp * sizeof(unsigned char));
	B = (unsigned char *)malloc(tmp * sizeof(unsigned char));
	tmp = tmp*4;
	Raw = (unsigned char *)malloc(tmp *sizeof(unsigned char));
